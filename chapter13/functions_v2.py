...
# The drawdown-duration routines below count rows, not calendar days. What one
# row represents depends on how the caller resampled the returns, so we map the
# annualization factor to the matching unit instead of assuming daily data.
_PERIOD_UNITS = {252: 'days', 52: 'weeks', 12: 'months', 4: 'quarters', 1: 'years'}


def max_drawdown_duration(drawdown_series):
    """
    Calculate the longest stretch spent below a prior peak.

    The count is in periods, not calendar days: one period is one row of the
    series you pass in. Daily returns give days, monthly returns give months.
    ...
    -------
    int
        Longest number of consecutive periods spent below a prior peak
    """
    duration = 0
    max_duration = 0
    ...
    for dd in drawdown_series:
        if dd < 0:
            # Still underwater: add another period
            duration += 1
...
def find_drawdown_episodes(cumulative_returns, drawdown_series, top_n=5,
    ...
    period_unit : str
        Label for the duration column. Durations are counted in rows of the
        series you pass in, so pass 'days' for daily data, 'months' for
        monthly data, and so on.

    Returns
    -------
    pd.DataFrame
        Columns: Start, Trough, Recovery, Depth, Duration (<period_unit>)
    """
    duration_col = f'Duration ({period_unit})'
    episodes = []
    in_drawdown = False
    start = None
...
def calculate_performance_metrics(returns, benchmark_returns=None,
    ...
    # Longest stretch spent below a prior peak. We call the helper defined
    # above rather than repeating its loop here, so the logic lives in exactly
    # one place. The count is in periods, matching whatever the caller
    # resampled to.
    max_duration = max_drawdown_duration(drawdown)

    # Ulcer Index: root mean square of the drawdown series, again reusing the
    # helper above instead of duplicating the calculation.
    ulcer = ulcer_index(drawdown)

    # --- Risk-adjusted metrics ---
    sharpe = (ann_return - risk_free_rate) / ann_vol if ann_vol > 0 else 0
    sortino = (ann_return - risk_free_rate) / downside if downside > 0 else 0
    calmar = ann_return / abs(max_dd) if max_dd != 0 else 0
    # UPI uses annualized return in percentage points to match the units of
    # the Ulcer Index, so the risk-free rate has to be scaled the same way.
    # Both arrive here as decimals, and mixing the two would silently subtract
    # 0.02 from a figure expressed in percentage points.
    upi = (ann_return * 100 - risk_free_rate * 100) / ulcer if ulcer > 0 else 0
    ...
    # Label the duration with the unit it was actually counted in, so monthly
    # metrics do not report months under a "days" heading.
    period_unit = _PERIOD_UNITS.get(periods_per_year, 'periods')
    ...
    metrics = {
        ...
        f'Max DD Duration ({period_unit})': max_duration,
